...
te_index = readRDS("teindex.rds")
tr_index = readRDS("trindex.rds")
...
# freq:mse=0.9594, rmse = 0.9795234
#occurs:mse=0.9395003, rmse = 0.9692782
xgb <- xgboost(data = trmat, label = y_tr, nrounds = 50)
y_pre = predict(xgb, temat)
(mse = sum((y_pre-y_te)^2) / length(y_te))
(rmse = sqrt(mse))
...
maxent_df <- data.frame(
  true_rating = as.numeric(train$stars[test_range]),
...
incrrct1_index = test_range[result_df$true_rating == 1 & result_df$maxent_rating != 1]# index returned are index in "train"
